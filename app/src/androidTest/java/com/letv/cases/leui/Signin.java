...
/*        String[] names={"推荐","美食","北京","社会","推荐"};
    ...
    public void testWeiLiSearch() throws UiObjectNotFoundException, RemoteException {
        ...
        if(tosearch!=null) {
            for (int k = 0; k < 30; k++) {
                ...
                phone.click((int) (phone.getDisplayWidth() * 0.5), (int) (phone.getDisplayHeight() * 0.38));
                sleepInt(5);
                for (int j = 0; j < 2; j++) {
                    sleepInt(3);
                    phone.swipe(600 + getRandom(200), 1340 + getRandom(100), 600 + getRandom(200), 800 + getRandom(100), getRandom(150));
    ...
    public void testKuaiMaSearch() throws UiObjectNotFoundException, RemoteException {
        ...
        sleepInt(1);
        press_back(1);
        touTiao = phone.findObject(By.text("小报"));
        if (touTiao != null) {
            touTiao.click();
            sleepInt(1);
        }
...
/*                if(Build.DEVICE.equals("ja3gduosctc")) {
    ...
    public void testStatusBar() throws UiObjectNotFoundException {
        ...
        sleepInt(2);
        List<UiObject2> news=phone.findObjects(By.text(Pattern.compile(".*[阅读.*金币].*")).pkg("com.jifen.qukan"));
        int n=news.size();
        for (int i=0;i<n;i++){
            sleepInt(2);
           // List<UiObject2> news=phone.findObjects(By.pkg(Pattern.compile("com.jifen.qukan")));
...
//        callShell("am start -n com.cashtoutiao/com.cashtoutiao.account.ui.main.MainTabActivity");
        ...
        String[] names={"推荐","热点","娱乐","健康","美文","笑话","育儿"};
        int name=getRandom1(names.length);
        Log.i(TAG, "testHuiTouTiao: name"+name+names[name]);
        ...
        for(int k=0;k<6;k++) {
            ...
            for (int q = 0; q < lists.size() - 1; q++) {
                ...
                if (a == null) {
                    ...
                    sleepInt(1);
                    for (int j = 0; j < 4; j++) {
                        phone.swipe((int) (phone.getDisplayWidth() * 0.5), (int) (phone.getDisplayHeight() * 0.7), (int) (phone.getDisplayWidth() * 0.5), (int) (phone.getDisplayHeight() * 0.2), 10);
                        sleepInt(3);
                        phone.swipe((int) (phone.getDisplayWidth() * 0.5), (int) (phone.getDisplayHeight() * 0.7), (int) (phone.getDisplayWidth() * 0.5), (int) (phone.getDisplayHeight() * 0.2), 10);
                        sleepInt(2);
                    }
    ...
    if(reading!=null){
        ...
        sleepInt(5);
        press_back(1);
        String[] names={"推荐","推荐","命理","情感","养生","娱乐","娱乐"};
        int name=getRandom1(names.length);
        UiObject2 type = waitForObj(By.text(names[name]));
        verify(names[name]+"not exists", type != null);
        type.click();
...
//        callShell("am start -n com.cashtoutiao/com.cashtoutiao.account.ui.main.MainTabActivity");
        ...
        int name=getRandom1(names.length);
        sleepInt(4);
        phone.swipe((int)(phone.getDisplayWidth()*0.12),(int)(phone.getDisplayHeight()*0.2),(int)(phone.getDisplayWidth()*0.12),(int)(phone.getDisplayHeight()*0.8),30);
        sleepInt(1);
        Log.i(TAG, "testHuiTouTiao: name"+name+names[name]);
        UiObject2 type = waitForObj(By.text(names[name]));
        verify(names[name]+"not exists", type != null);
        ...
        for(int k=0;k<15;k++) {
            ...
            for(int q=0;q<lists.size()-1;q++) {
                try {
                    ...
                    lists=phone.findObject(By.clazz("android.widget.ListView")).getChildren();
                }
                press_back(1);
            }
            phone.swipe((int) (phone.getDisplayWidth() * 0.5), (int) (phone.getDisplayHeight() * 0.8), (int) (phone.getDisplayWidth() * 0.5), (int) (phone.getDisplayHeight() * 0.2), 30);
